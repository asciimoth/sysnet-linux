...
func (c *Config) Write(w io.Writer, owner string) error {
	...
	io.WriteString( //nolint
		buf,
		"# resolv.conf(5) file generated by "+owner+"\n",
	)
	io.WriteString( //nolint
		buf,
	...
	for _, ns := range c.Nameservers {
		io.WriteString(buf, "nameserver ") //nolint
		io.WriteString(buf, ns.String())   //nolint
		io.WriteString(buf, "\n")          //nolint
	}
	...
	for scanner.Scan() {
		...
		if s, ok := strings.CutPrefix(line, "nameserver"); ok {
			...
			if len(nameserver) == len(s) {
				return nil, fmt.Errorf(
					"missing space after \"nameserver\" in %q",
					line,
				)
			}
		...
		if s, ok := strings.CutPrefix(line, "search"); ok {
			...
			if len(domains) == len(s) {
				// No leading space?!
				return nil, fmt.Errorf(
					"missing space after \"search\" in %q",
					line,
				)
			}
			for len(domains) > 0 {
				...
				if err != nil {
					return nil, fmt.Errorf(
						"parsing search domain %q in %q: %w",
						domain,
						line,
						err,
					)
