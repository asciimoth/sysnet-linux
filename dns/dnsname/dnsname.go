...
func ValidLabel(label string) error {
	...
	if len(label) > maxLabelLength {
		return fmt.Errorf(
			"%q is too long, max length is %d bytes",
			label,
			maxLabelLength,
		)
	}
	if !isalphanum(label[0]) {
		return fmt.Errorf(
			"%q is not a valid DNS label: must start with a letter or number",
			label,
		)
	}
	if !isalphanum(label[len(label)-1]) {
		return fmt.Errorf(
			"%q is not a valid DNS label: must end with a letter or number",
			label,
		)
	}
	if len(label) < 2 {
	...
	for i := 1; i < len(label)-1; i++ {
		if !isdnschar(label[i]) {
			return fmt.Errorf(
				"%q is not a valid DNS label: contains invalid character %q",
				label,
				label[i],
			)
		}
